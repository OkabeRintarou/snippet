...
int amdgpu_bo_map(amdgpu_device_handle dev, amdgpu_bo_handle bo_handle,
    ...
    r = amdgpu_bo_va_op_raw(dev, bo_handle, 0, ALIGN(size, getpagesize()), vmc_addr,
    ...
    r = amdgpu_bo_cpu_map(bo_handle, cpu);
    if (r)
        goto error_cpu_map;
...
error_cpu_map:
    *cpu = nullptr;
    amdgpu_bo_cpu_unmap(bo_handle);
error_va_map:
    amdgpu_bo_va_op(bo_handle, 0, size, vmc_addr, 0, AMDGPU_VA_OP_UNMAP);
error_va_alloc:
...
int amdgpu_bo_alloc_and_map_raw(amdgpu_device_handle dev, uint64_t size,
    ...
    r = amdgpu_bo_alloc_wrap(dev, size, alignment, heap, alloc_flags, &buf_handle);
    if (r)
        return r;

    r = amdgpu_bo_map(dev, buf_handle, size, alignment, mapping_flags, cpu, mc_address, va_handle);
    if (r)
        return r;
