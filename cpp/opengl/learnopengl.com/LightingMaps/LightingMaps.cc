...
int main() {
  ...
  Shader shader(shader_source::from_file,
                "../../Resources/Shader/LightingMaps_Vertex.glsl",
                "../../Resources/Shader/LightingMaps_Frag.glsl");
  if (!shader.is_valid()) {
  ...
  auto vao = builder.stride(8)
                 .add(3).add(3).add(2)
                 .data(vertices, sizeof(vertices)).build();
  ...
  Texture2D light_map("../../Resources/Textures/container2.png");
  if (!light_map.is_valid()) {
    std::cerr << "Fail to load texture: " << light_map.message() << std::endl;
    return -1;
  }
  Texture2D specular_map("../../Resources/Textures/container2_specular.png");
  if (!specular_map.is_valid()) {
    std::cerr << "Fail to load texture: " << specular_map.message() << std::endl;
    return -1;
  }
  ...
  shader.set_float("material.shininess", 64.f);
