...
static int remap_pfn_mmap(struct file *file, struct vm_area_struct *vma) {
	...
	unsigned pfn_start = page_to_pfn(start_page) + vma->vm_pgoff;
	unsigned long virt_start = (unsigned long)page_address(start_page);
	unsigned long size = vma->vm_end - vma->vm_start;
	int ret = 0;

	pr_info("phy: 0x%lx, offset: 0x%lx, size: 0x%lx\n", (unsigned long)(pfn_start << PAGE_SHIFT), offset, size);

	ret = remap_pfn_range(vma, vma->vm_start, pfn_start, size, vma->vm_page_prot);
	if (ret)
		pr_info("%s: remap_pfn_range failed at [0x%lx 0x%lx]\n",
...
static int __init remap_pfn_init(void) {
	...
	pr_info("pages start virtual address: %lx, start physical address: %lx\n",
			(unsigned long)page_address(start_page),
			(unsigned long)(page_to_pfn(start_page) << PAGE_SHIFT));
