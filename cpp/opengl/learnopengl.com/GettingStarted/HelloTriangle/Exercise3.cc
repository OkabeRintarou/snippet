...
int main() {
  ...
  glShaderSource(firstFragmentShader, 1, &fragmentShaderSource, nullptr);
  glCompileShader(firstFragmentShader);
  glGetShaderiv(firstFragmentShader, GL_COMPILE_STATUS, &success);
  if (!success) {
    glGetShaderInfoLog(firstFragmentShader, 512, nullptr, infoLog);
